    ...
    for qdate in quarters:
        ...
        if isinstance(bs, pd.DataFrame) and isinstance(inc, pd.DataFrame):
            ...
            if len(bs_dates) > 0 and len(inc_dates) > 0:
                ...
                ltd = bs_row.get('Long Term Debt', np.nan)
                ...
                if shares_col in bs_row.index and not pd.isna(bs_row[shares_col]):
                    mcap = rec['close_price'] * bs_row[shares_col]
...
print(f"  ✓ Train: {len(X_train):>5d} samples  ({y_train.sum():>3d} distress)  2015–2021")
print(f"  ✓ Val:   {len(X_val):>5d} samples  ({y_val.sum():>3d} distress)  2022")
...
def altman_predict(X_df):
    ...
    risk = -z
    prob = 1 / (1 + np.exp(-0.8 * (risk + 2.0)))
    prob[nan_mask] = 0.3
    pred = (z < 1.81).astype(int)
    return prob, pred
